...
class ApplicationController extends AppController{
    ...
    private $DmiChangeFinalSubmits;
    private $DmiChangeSelectedFields;
    private $DmiChangeFieldLists;
    private $DmiUsers;
	...
	public function initialize(): void
	{
        ...
        $components = [
            'Romoioapplicantcommunicationactions',
            'Customfunctions',
            'Authentication',
            'Paymentdetails',
            'Communication',
    ...
    private function loadAllModels(): void {
        ...
        $this->DmiRenewalSubmissionLogs = $this->AqcmsWrapper->customeLoadModel('DmiRenewalSubmissionLogs');
        ...
        $this->DmiChangeFirms = $this->AqcmsWrapper->customeLoadModel('DmiChangeFirms');
        ...
        $this->DmiChangeAllTblsDetails = $this->AqcmsWrapper->customeLoadModel('DmiChangeAllTblsDetails');
        ...
        $this->DmiChangeFinalSubmits = $this->AqcmsWrapper->customeLoadModel('DmiChangeFinalSubmits');
        $this->DmiChangeSelectedFields = $this->AqcmsWrapper->customeLoadModel('DmiChangeSelectedFields');
        $this->DmiChangeFieldLists = $this->AqcmsWrapper->customeLoadModel('DmiChangeFieldLists');
	...
	public function changeRequest() {
        ...
        //updated cond. in below query, added $grantDateCondition on 05-02-2024 by Amol, to solve rejct appl. issue
		$checkIfInProcess = $this->DmiChangeFinalSubmits->find('all',array('conditions'=>array('customer_id'=>$customer_id,$grantDateCondition),'order'=>'id desc'))->first();
	...
	public function applicationForCertificate() {
		...
		if($application_type == 11){
            ...
            $reportStatus = $this->DmiBgrCommodityReports->find('all',array('conditions'=>array('customer_id IS'=>$customer_id,'form_status'=>'approved'),'order'=>'id DESC'))->first();
	...
	//	}
		...
		//added call to function to check MSEO or BEVO, on 12-09-2023 by Amol
        ...
        $section_model = $this->AqcmsWrapper->customeLoadModel($section_model);
		...
		if (null !== $this->request->getData('save')) {
			...
			if (is_array($result)=='') {
				...
				if ($result == 1) {
					...
					// This message is changed for the Surrender module (SOC) - Akash Thakre [12-05-2023]
					if ($application_type == 9) {
					...
					#This has been added to show the specific message if the application is for the lab export renewal - LXR : Akash [28-08-2023] : [LIVE : 01-12-2023]
					elseif ($application_type == 13) {
						$message = "Application of Renewal for Labarotaory Export " .' - '.ucwords(str_replace('_',' ',$section_details['section_name'])).' section, '.$process_query.' successfully';
					}
					else{
			...
			if (!empty($this->request->getData('once_no'))) {
				...
				if ($final_submit_call_result == true) {
					...
					#This has been added to show the specific message if the application is for the lab export renewal - LXR : Akash [28-08-2023]
					elseif ($application_type == 13) {
						$message = "Application of Renewal for Labarotaory Export is final Submitted Successfully";
					} elseif($application_type == 11){ # added for BGR module by shankhpal
						$message = "Application of biannual grading report final Submitted Successfully";
					}
					else{
					...
					if ($application_type == 4 && (empty($is_training_completed)||$is_training_completed!='no')) {
	...
	public function payment() {
		...
		if (!empty($customer_id)) {
			...
			//added call to function to check MSEO or BEVO, on 08-03-2024 by Amol
			$MseoOrBevo = $this->Customfunctions->checkMseoOrBevo($customer_id);
			$this->set('MseoOrBevo',$MseoOrBevo);
            ...
            $payment_table = $this->AqcmsWrapper->customeLoadModel($payment_table);
			...
			if($application_type == 4){
				...
				$chemist_reg_details = $this->DmiChemistRegistrations->find('all', array('fields'=>['payment','is_training_completed'], 'conditions'=>['chemist_id IS'=>$customer_id]))->first();
			   $this->set('payment_amt',$chemist_reg_details['payment']);
			   $is_training_completed=$chemist_reg_details['is_training_completed'];
			   $this->set('application_charge',$chemist_reg_details['payment']);
			...
			if (null !== ($this->request->getData('final_submit'))) {
				...
				if (!empty($this->request->getData('once_no'))) {
					...
					if ($final_submit_call_result == true) {
						...
						if ($application_type == 9) {
							$message = "Application of Surrender for ".$firm_type_text.' - final submitted successfully';
						}#This has been added to show the specific message if the application is for the lab export renewal - LXR : Akash [28-08-2023] : [LIVE : 01-11-2023]
						elseif ($application_type == 13) {
							$message = "Application of Renewal for Labarotaory Export is final submitted successfully";
						}elseif ($application_type == 11){
							$message = "Application of Commodity wise report is final Submited Successfully";
						}
						else{
							$message = $firm_type_text.' - final submitted successfully';
						}
                       ...
                       //if application type 4 rediirect to chemist home after final submit-Laxmi[30-05-23]
						...
						if(!empty($appl_type) && $appl_type == 4 && $is_training_completed!='no'){
							$redirect_to = '../chemist/home';
						...
						//Added this call to save the user action log on 04-03-2022 by Akash Thakre
	...
	public function applicationFinalSubmit() {
		...
		if ($final_submit_call_result == true) {
		   ...
		   //After final submitted redirect to home not pdf added by laxmi B. on 30-05-2023
			$application_type = $this->Session->read('application_type');
          //Joshi, Akash - Need to generate PDF if training is required in chemist realm [22-11-2023]
			$is_training_completed=$this->Session->read('is_training_completed');
			if(!empty($application_type) && $application_type == 4 && (empty($is_training_completed) || $is_training_completed!='no')){
				$redirect_to = '../chemist/home';

			}

			if(!empty($application_type) && $application_type == 11){ // $application_type == 11 added by shankhpal for BGR module on 14/11/2023
				$redirect_to = '../chemist/display-mapped-c-a-dropdown';
