...
class BeforepageloadComponent extends Component {
	...
	public array $components = array('Session','Authentication');
	...
	public function setLogoutTime() {
		...
		if ($this->Session->read('username') != null) {
			...
			if (filter_var($username_id, FILTER_VALIDATE_EMAIL)) {
				...
				$find_id_list = $DmiUserLogs->find('list', array('valueField'=>'id','conditions'=>array('email_id IS'=>$username_id)))->all()->toList();
				...
				$find_id_list = $DmiCustomerLogs->find('list', array('valueField'=>'id','conditions'=>array('customer_id IS'=>$username_id)))->all()->toList();
	...
	public function get_all_concent_messages() {
		...
		foreach ($get_concent_msg as $each_msg) {
			...
			if ($each_msg['concent_for']=='ca_mseo') {//added on 12-09-2023 for MSEO updates

                $ca_mseo = $each_msg['message'];
            }
		}
		...
		$this->Controller->set(compact('esign_msg','aadhar_auth_msg','ca_new_msg','ca_renewal_msg','ca_bevo_new_msg',
									...
									'esign_please_wait','without_esign','ca_mseo'));
	}
	...
	public function showButtonOnSecondaryHome() {
		...
		// the not empty condition is added to the entire code : Akash Thakre [14-09-2023] : [LIVE : 01-12-2023]
		if (!empty($customer_id)) {

			//Load model
			$DmiFinalSubmits = TableRegistry::getTableLocator()->get('DmiFinalSubmits');

			//For Lab Export Renewal Application (LXR) : Akash Thakre [14-09-2023]
			...
			if ($export_unit_status == 'yes' && $firm_type == 3) {
            ...
            $this->Controller->set('export_unit_status',$export_unit_status); // set the export unit value causing the undefined variable issue : Akash Thakre [13-03-2024]
	...
	public function checkValidRequest(){
		...
		/*	$validHostName = array('agmarkonline.dmi.gov.in','esignservice.cdac.in');
			...
			//Added on dt. 09-10-2023 by Jayant and Amol to avoid site access through IP
			$validHostName = array('10.153.72.52','164.100.115.134');
			$hostName = $_SERVER['HTTP_HOST'];
			if(in_array($hostName,$validHostName)){
				echo "Direct IP access is not allowed.";
				exit;
			}
