...
class CustomfunctionsComponent extends Component {
    ...
    public function initialize(array $config): void {
        ...
        $this->components = ['Session','Randomfunctions'];   // for solving error [Type of app\Controller\Component\CreatecaptchaComponent::$components must be array (as in class Cake\Controller\Component)] added by shankhpal on 26-03-2024
    ...
    public function firmType($customer_id) {
        ...
        $split_customer_id = explode('/',(string) $customer_id); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [05-03-2024]
    ...
    public function checkCaBevo($customer_id) {
        ...
        if ($split_customer_id[1] == 1) {
            ...
            //added id '11' on 05-09-2022 for Fat Spread updates after UAT
            //added id '22' on 12-09-2023 for MSEO updates
            if ($check_application_type['commodity'] == 106 || $check_application_type['commodity'] == 11
                || $check_application_type['commodity'] == 22) {
                $applicant_bevo = 'yes';
            } else {
                $applicant_bevo = 'no';
            }
    ...
    public function formStatusValue($sections,$customer_id) {
        ...
        if(isset($_SESSION['current_level']) && $_SESSION['current_level'] == 'applicant' && $application_type == 11) {
            // added by shankhpal on 14/11/2023 for BGR Module
            $customer_id = $this->Session->read('packer_id');
        }
        $return_value = 1; // if all section "saved"
        $form_save_count = 0;
        $form_approve_count = 0;
        //variable set to check chemist training alredy done or not by laxmi on 22-09-2023
        $registeredChemist = "";
        $chemistRegistration = TableRegistry::getTableLocator()->get('DmiChemistRegistrations');
        $checkTrainingDone = $chemistRegistration->find('all')->where(['chemist_id IS'=>$customer_id])->first();
        ...
        //added this cond. on 19-10-2023 by Amol
        //to set payment_section for chemist appl. conditionally as all sections have payment models in flow table
        //it was conflicting to show final submit button
        if($application_type == 4 ){
            if(!empty($checkTrainingDone) && $checkTrainingDone['is_training_completed'] == 'no'){
                $payment_table = $sections[0]['payment_section'];
            }else{
                $payment_table = '';
            }
        ...
        if ($payment_table != "" && ($oldapplication=='no' || $application_type != 1)) {
        ...
        if((!empty($checkTrainingDone) && ($checkTrainingDone['is_training_completed'] == '' || $checkTrainingDone['is_training_completed'] == 'yes')) && $section_form_status == 'saved'){
            $return_value = 1;
            $registeredChemist =1;
           }
    ...
    public function applicationFinalSubmitted($customer_id,$all_section_status) {
        ...
        if ($all_section_status == 1) {
            ...
            $DmiBgrSmsEmailTemplate = TableRegistry::getTableLocator()->get('DmiBgrSmsEmailTemplate'); // load model for BGR by shankhpal
            ...
            */
            ...
            /*
                for module bianually grading report when chemist final submit application
                save customer_id -> CA insted of chemist id
            */
            ...
            if (!empty($final_submit_entry_id)) {
                ...
                if ($Dmi_final_submit->save($Dmi_final_submit_Entity)) {
                    ...
                    if ($application_type == 4) {
                        ...
                        #SMS: Chemist final submitted.
                        $DmiBgrSmsEmailTemplate->sendMessage(6,$customer_id); #SMS RO
                        $DmiBgrSmsEmailTemplate->sendMessage(4,$chemist_id); #SMS Chemist
                        $DmiBgrSmsEmailTemplate->sendMessage(5,$customer_id); #SMS Applicant
                    }else{
                ...
                if ($Dmi_final_submit->save($Dmi_final_submit_Entity)) {
                    ...
                    if ($paymentSection == 'available') {
                        ...
                        $firm_type = $this->firmType($customer_id);
                        if ($firm_type==3 || $application_type == 11) { // added  $application_type == 11 by shankhpal on 14/11/2023 for BGR module
    ...
    public function checkApplicantExportUnit($customer_id) {
        ...
        if (!empty($_SESSION['application_type']) && !empty($_SESSION['packer_id']) && $_SESSION['application_type'] == 4) {
    ...
    public function getApplicationCurrentStatus($customer_id,$appl_type=null) {
        ...
        //For Lab Export Renewal (LXR) : Application Type 13 - Akash Thakre [20-09-2023] : : [LIVE : 01-12-2023]
        //If the firm is lab and the export unit is yes , then below details of the renewal will be fetched
        //from the diffrent table named DmiLxrFinalSubmit which is made for the lab export renewal
        $export_unit_status = $this->checkApplicantExportUnit($customer_id);

        $firmType = $this->firmType($customer_id);

        if ($export_unit_status == 'yes' && $firmType == 3) {
            $DmiRenewalFinalSubmits = 'DmiLxrFinalSubmits';
        } else {
            $DmiRenewalFinalSubmits = 'DmiRenewalFinalSubmits';
        }

        //Load model in the variable to use the load model dynamically : Akash Thakre [20-09-2023]
    ...
    public function checkApplicantValidForRenewal($customer_id) {
        ...
        //only for Lab export
        if($exportUnit=='yes' && $split_customer_id[1] == 3){
            $DmiRenewalFinalSubmits = TableRegistry::getTableLocator()->get('DmiLxrFinalSubmits');
        }else{
            $DmiRenewalFinalSubmits = TableRegistry::getTableLocator()->get('DmiRenewalFinalSubmits');
        }
        ...
        if (!empty($list_id)) {
            ...
            */
            ...
            //updated applied as per the Lab export renewal order
            //updates applied on 14-09-2023 by Akash : [LIVE : 01-12-2023]
            $exportUnit = $this->checkApplicantExportUnit($customer_id);
            //only for Lab export
            if($exportUnit=='yes' && $split_customer_id[1] == 3){
                //updated logic for extension date with + 1 month for lab export appl. by Amol on 14-11-2023
                $ext_dt = date('d/m', strtotime("+1 month",strtotime($certificate_validity_date)));
            }
            //for CA Application
            if ($split_customer_id[1] == 1) {
    ...
    public function getCertificateValidUptoDate($customer_id,$cert_grant_date) {
        ...
        $grant_date_pattern = str_replace('/','-',(string) $cert_grant_date); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
        $new_order_date_pattern = str_replace('/','-','01/04/2021');

        $split_grant_date = explode(' ', (string) $grant_date_pattern);//as it will come with 00:00:00 some time
        ...
        if ($g_date < $n_date) { //if provided grant date is less than new order date '01-04-2021'
            ...
            if (!empty($check_grant_entry)) {
                ...
                if ($check_grant_entry['pdf_version']==1 && !empty($check_appl_is_old)) {
                    ...
                    if(empty($renewa_dates_present) && ($split_customer_id[1] == 1 || $split_customer_id[1] == 3)){
                        ...
                        $cert_grant_date = str_replace('/','-',(string) $cert_grant_date); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
            ...
            //for Laboratory Application
                ...
                //updates applied on 19/01/2024 by Amol
                $exportUnit = $this->checkApplicantExportUnit($customer_id);
                //only for Lab export
                if($exportUnit=='yes'){
        ...
        //also if lab is NABL accreditated then valid upto accreditation
            ...
            //this variable is added on 13-12-2023 by Amol, modified and used in below cond.
            //to decide month cond. should be use or not while getting validity below
            $checkMonthCond = 'yes'; //default value is yes
            ...
            //conditions updated on 18-10-2023 for CA and PP
            ...
            if (!empty($check_app_subm)) {
                ...
                if ($split_customer_id[1] == 1 || $split_customer_id[1] == 3){ //previously only for Lab appl, for CA cond. added on 18-10-2023
                    $get_grant_year = date('Y',strtotime($app_sub_date));
                    //added this variable here on 12-01-2024, as for CA and Lab validity must be ren. appl. year + 5
                    $checkMonthCond = 'no';

                 //for PP cond. added on 18-10-2023, to calculate grant if granted after 1-2 years of appl
                }elseif($split_customer_id[1] == 2){
                    $app_sub_month = date('m',strtotime($app_sub_date));
                    $app_sub_year = date('Y',strtotime($app_sub_date));

                    //if ren. applied in oct,nov,dec and grant after next year ex. appl in 2022 and grant 2024
                    if(($app_sub_month > 9) && ($get_grant_year - $app_sub_year) > 1){
                        $get_grant_year = $app_sub_year+1;
                    //if ren. applied in Jan. and grant in next year ex. appl in 2022 and grant 2023
                    }elseif(($app_sub_month < 9) && ($get_grant_year - $app_sub_year) > 0){
                        $get_grant_year = $app_sub_year;
                    }

                }

            } else {
                //only when CA and lab, for first grant, added if cond. on 03-11-2023 by Amol
                if($split_customer_id[1] == 1 || $split_customer_id[1] == 3){
                    ...
                    if(!empty($check_app_subm)){ // this line added by shankhpal shende on 12/09/2022
                        $app_sub_date = $check_app_subm['created'];
                        $app_sub_date = str_replace('/','-',$app_sub_date);
                        $get_app_year = date('Y',strtotime($app_sub_date));//variable name changed on 13-12-2023

                        //as per DMI order, new appl. validity should be calculated from year of grant.13-12-2023

                        //this cond. is added on 13-12-2023 by Amol,
                        //if app year and grant year for new appl. is not same do not use month cond.
                        //commented on 12-01-2024 by Amo, as not required for new appl. validity will as per grant year and month
                        /*if($get_app_year != $get_grant_year){
            ...
            if ($split_customer_id[1] == 1)//for CA Application
            {
                //cond. modified on 13-12-2023 by Amol, && $checkMonthCond=='yes' added
                if ($get_grant_month <= 3 && $checkMonthCond=='yes')
                {
                    $valid_upto_year = $get_grant_year + 4;
                } else {
                    $valid_upto_year = $get_grant_year + 5;
                ...
                //cond. modified on 13-12-2023 by Amol, && $checkMonthCond=='yes' added
                if ($get_grant_month <= 6 && $checkMonthCond=='yes')
                {
                    $valid_upto_year = $get_grant_year + 4; //now validity for 5 years as per new order
                } else {
    ...
    public function customerLastLogin() {
        ...
        //check User Type
        $explodeValue = explode('/',(string) $userType);  // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
    ...
    public function getEmailMasked($email_id) {
        ...
        $em   = explode("@",(string) $email_id); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
    ...
    public function fileUploadLib($file_name,$file_size,$file_type,$file_local_path) {
        ...
        $get_extension_value = explode('.',(string) $file_name); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
        ...
        //updated code to add limit 5mb for lab exp. ren. appl. uploads
        //on 26-10-2023 by Amol : [LIVE : 01-12-2023]
        $file_size_limit = 2097152;
        $file_size_limit_msg = "File too large. File must be less than 2 megabytes.";
        if($this->Session->read('application_type')=='13'){
            $file_size_limit = 5242880;
            $file_size_limit_msg = "File too large. File must be less than 5 megabytes.";
        }

        if ($file_size > $file_size_limit) {
            $this->Controller->customAlertPage($file_size_limit_msg);
            ...
            $fileNameFiltered = preg_replace('!\s+!', '_', $file_name); // replace spaces with "_" to handle "tcpdf file link not found" issue - Akash Thakre [29-11-2023]
            $fileNameFiltered = str_replace("'", "", $fileNameFiltered); // remove apostrophe "'" as this causing add more html table fetching issue - Akash Thakre [29-11-2023]
    ...
    public function checkFatSpreadOrBevo($customer_id) {
        ...
        if ($this->checkCaBevo($customer_id) == 'yes') {
            ...
            $sub_commodities_array = explode(',',(string) $sub_commodities_values); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
            ...
            foreach ($sub_commodities_array as $each_sub_commodity) {
                ...
                //changed fron 172 to 173 on 11-07-2019 by Amol
                //added condition for MSEO with '245' on 12-09-2023 by Amol
                if ($each_sub_commodity == '173' || $each_sub_commodity == '245')
                {
                    $bevo = 'yes';
                }
    ...
    public function dateFormatCheck($date) {
        ...
        if (!empty($date)) {
            ...
            $input_date = explode('/',(string) $date); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
            $removeTime    = explode(' ',(string) $input_date[2]); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
            $year = $removeTime[0];
    ...
    public function showOldCertDetailsPopup($customer_id) {
        ...
        $sub_comm_id = explode(',',(string) $added_firm_field['sub_commodity']); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
    ...
    public function returnGrantDateCondition($customer_id,$application_type=null) {//new argument added on 13-04-2023 "$application_type"
        ...
        if ($application_type == 3) {
            ...
            //below cond. added to check if appl is rejected, then to check rejected date cond. instead of grant,
            //on 05-02-2024 by Amol, to resolve issue if previous appl. rejected applicant should able to apply again as fresh.
            $rejectedApplTable = TableRegistry::getTableLocator()->get('DmiRejectedApplLogs');
            if (!empty($grantDate)) {
                $rejectedDate = $rejectedApplTable->find('all',array('conditions'=>array('customer_id IS'=>$customer_id,'date(created) >'=>$grantDate['created'],'appl_type IS'=>3),'order'=>'id DESC'))->first();
            }else{
                $rejectedDate = $rejectedApplTable->find('all',array('conditions'=>array('customer_id IS'=>$customer_id,'appl_type IS'=>3),'order'=>'id DESC'))->first();
            }
            if(!empty($rejectedDate)){
                $grantDate = $rejectedDate;
            }
         ...
         elseif ($application_type == 10) { //added on 30-05-2023 by shankhpal to get RTI grant date
             ...
             $grantDate = $DmiRtiFinalReports->find('all',array('conditions'=>array('customer_id IS'=>$customer_id,'current_level'=>'level_3_','status'=>'approved'),'order'=>'id DESC'))->first();
            if(!empty($grantDate)){ //RTI Issue Pending application updated on 25/11/23 if granted application exixt retunr null
                return null;
            }
        }
    ...
    public function changeDateFormat($date) {
        ...
        if (!empty($date)) {
            ...
            $result    = explode(' ',(string) trim($date)); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
    ...
    public function inspRequiredForChangeApp($customer_id,$appl_type) {
        ...
        if ($appl_type == 3) {
            ...
            if ($selectedfields != null) {
                $selectedValues = explode(',',(string) $selectedfields['changefields']); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
            }
    ...
    public function applicationCharges($applicationType,$firmType) {
        ...
        if ($applicationType == 3 &&  $firmType == 1) {
            ...
            if (in_array(3,$selectedValues[0])) {
                ...
                $changeCommoditiesDetails = explode(',', (string) $changeFirmDetails['sub_commodity']); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
                $commoditiesDetails = explode(',', (string) $firmDetails['sub_commodity']); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
    ...
    public function showCommdityInApplList($sub_commodities) {
        ...
        $sub_commodity_array = explode(',', (string) $sub_commodities); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
        $MCommodity = TableRegistry::getTableLocator()->get('MCommodity');
    ...
    function old_app_approved_by($customer_id) {
        ...
        $explode = explode("/", (string) $customer_id); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
    ...
    public function replicaAllotedListCall($show_list_for) {
        ...
        if (strpos(base64_decode($username),'@')) {//for email encoding
            ...
            $explodeValue = explode('/', (string) $username); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
    ...
    public function findOfficerCountInoffice($user) {
        ...
        if (strrpos(base64_decode($user),'@')) {//for email encoding
            ...
            $explodevalue = explode('/', (string) $user); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
    ...
    public function getShortCode($customer_id) {
        ...
        $explodevalue = explode('/', (string) $customer_id); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
    ...
    public function getQrCode($result,$type=null){
        ...
        if($type == 'SOC'){ # For Surrender Flow (SOC)
            ...
            $split_customer_id = explode('/', (string) $result[0]); // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [06-03-2024]
            if ($split_customer_id[1] == 1) {
    ...
    public function isApplicationRejected($username,$appl_type=null){
        ...
        $stmt1 = $conn->execute("SELECT t1.* FROM dmi_rejected_appl_logs t1 Inner JOIN (SELECT MAX(id) AS id FROM dmi_rejected_appl_logs GROUP BY customer_id,appl_type HAVING $condition) t2 ON t1.id= t2.id WHERE  t1.is_appeal_granted IS NULL or t1.is_appeal_granted = 'no' ");
            ...
            foreach ($flow_wise_tables as $eachflow) {
                ...
                foreach ($level_arr as $eachLevel) {
                    ...
                    foreach ($checkCurPosition[$i][$j] as $eachAppl) {
                        ...
                        $lastModified = explode(' ', (string) $eachAppl['modified'])[0]; // added the (string) type-cast to fix the PHP8.1.4 Depractions - Akash [05-03-2024]
    ...
    //method to differentiate bet. MSEO and BEVO, on 12-09-2023 by Amol
    public function checkMseoOrBevo($customer_id) {
        ...
        if ($this->checkCaBevo($customer_id) == 'yes') {
            ...
            foreach ($sub_commodities_array as $each_sub_commodity) {
                if ($each_sub_commodity == '173'){$bevo = 'yes';}
                elseif ($each_sub_commodity == '245'){$mseo = 'yes';}
            }

            $applicant_type='';
            if ($bevo == 'yes'){$applicant_type = 'bevo';}
            elseif ($mseo == 'yes') {$applicant_type = 'mseo';}
