...
*/
	...
	// New created for the Master of Manuals 
	// @Akash Thakre
	// Date : 08-12-2023

	public function addEditManualMaster($postData,$record_id=null){
	
		$username = $this->Session->read('username');

		//html encoding
		$manual_name = htmlentities($postData['manual_name'], ENT_QUOTES);
		$for = htmlentities($postData['for'], ENT_QUOTES);
		
		if(!empty($postData['link']->getClientFilename())){
					
			$file_name = $postData['link']->getClientFilename();
			$file_type = $postData['link']->getClientMediaType();
			$file_local_path = $postData['link']->getStream()->getMetadata('uri');
			...
			if (($file_type != "application/pdf")) {
				...
				$uploadPath = $_SERVER["DOCUMENT_ROOT"] . '/writereaddata/DMI/manuals/' . $for . '/';
				$uploadFile = $uploadPath.$fileNameFiltered;
				$uploadData = '';
			
				if (move_uploaded_file($file_local_path,$uploadFile)) {
					$uploadData = '/writereaddata/DMI/manuals/' . $for . '/' . $fileNameFiltered;
				} else {
		...
		$DmiManuals = TableRegistry::getTableLocator()->get('DmiManuals');

		$entity = $DmiManuals->newEntity($data_array);

		if ($DmiManuals->save($entity)) {
