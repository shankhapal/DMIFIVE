...
class RandomfunctionsComponent extends Component {
	...
	public function get_rejected_appl($each_flow){
		...
		if (!empty($stmt)) {
			...
			$innerResult = $query->all();

			// Extract customer_id values into an array
				foreach ($innerResult as $result) {
			...
			if (!empty($allocation_list)) {
				...
				foreach ($allocation_list as $allocated_id) {
					...
					if (!empty($allocation_details)) {
						...
						//This below condtion is added to check the apploication is rejected if it is rejected then exclude it from the array - Akash Thakre [06-09-2023]
						$is_application_rejected = $DmiRejectedApplLogs->find('all')->select(['customer_id'])->where(['customer_id IS ' => $allocated_customer_id,'appl_type' => $each_flow['application_type']])->order('id DESC')->first();
			...
			if (!empty($ho_mo_allocation_list)) {
				...
				foreach ($ho_mo_allocation_list as $ho_mo_allocated_id) {
					...
					if (!empty($ho_mo_allocation_details)) {
						...
						//This below condtion is added to check the apploication is rejected if it is rejected then exclude it from the array - Akash Thakre [06-09-2023]
						$is_application_rejected = $DmiRejectedApplLogs->find('all')->select(['customer_id'])->where(['customer_id IS ' => $ho_mo_allocated_customer_id,'appl_type' => $each_flow['application_type']])->order('id DESC')->first();
						if (empty($is_application_rejected)) {
	...
	//29-12-2022 by Amol
    //new parameter $rejcondition is added on 05-02-2024 by Amol
	public function setChangedDetailsForGrantPdf($customer_id,$customer_firm_data,$premises_data=null,$laboratory_data=null,$business_type=null,$rejcondition=null){
		...
		if (!empty($changeApplDetails[0]['commodity'])) {//if commodity changed
			...
			$commodity_name_list = $MCommodityCategory->find('all',array('conditions'=>array('category_code IN'=>$unique_commodity_id)))->toArray();
		...
		if (!empty($changeApplDetails[0]['premise_street'])) {//if premises changed
            ...
            //added this explode and if cond. on 07-02-2024 by Amol,
            //as $premises_district_name variable in CA and PP grant cert. used differently
            ...
            if($split_customer_id[1]==1){
                ...
                $this->Controller->set('premises_district_name',$premises_district_name['district_name']);
            }
        ...
        //the $rejcondition is added on 05-02-2024 by Amol, in below query
		$DmiChangeAllTblsDetails = TableRegistry::getTableLocator()->get('DmiChangeAllTblsDetails');
		$get_tbls_details = $DmiChangeAllTblsDetails->find('all', array('conditions'=>array('customer_id IS'=>$customer_id,$rejcondition,'delete_status IS NULL','status IS NULL'),'order'=>'id'))->toArray();
		...
		$added_directors_details = $DmiChangeDirectorsDetails->find('all', array('conditions'=>array('customer_id IS'=>$customer_id,$rejcondition,'delete_status IS NULL','status IS NULL'),'order'=>'id'))->toArray();
	...
	public function showChangedFieldsInGrantPdfSection($customer_id,$getNoOfAppl){
		...
		foreach($getNoOfAppl as $each){
			...
			if (!empty($getChangeDetails[$applCnt]['comm_category'])) {
				...
				$change_commodity_name_list[$applCnt] = $MCommodityCategory->find('all',array('conditions'=>array('category_code IN'=>$unique_commodity_id)))->toArray();
	...
	public function showChangedFieldsInApplPdf($customer_id){
		...
		if (!empty($getChangeDetails['comm_category'])) {
			...
			$change_commodity_name_list = $MCommodityCategory->find('all',array('conditions'=>array('category_code IN'=>$unique_commodity_id)))->toArray();
	...
	// Update : Function is added condtions to check if the NABL renewal date is present in the Database
	// and acccording to return the date : Akash Thakre [15-09-2023]
	//: [LIVE : 01-12-2023]
	...
	public function checkIfLabNablAccreditated($customer_id) {
		...
		// Check renewal details for laboratory export is present
		$renewalDetails = $DmiNablDates->getNablDate($customer_id);

		if (!empty($renewalDetails)) {
			return $renewalDetails['nabl_date'];
		}else{
			// Check if the applicant for laboratory selected NABL accreditation
			$checkNabl = $DmiLaboratoryOtherDetails->find()->where(['customer_id' => $customer_id])->orderDesc('id')->first();
