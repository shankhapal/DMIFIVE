...
class CustomersController extends AppController {
    ...
    private $DmiOldApplEsignCertLogs;
    private $DmiRtiReportPdfRecords;
    private $DmiSurrenderGrantCertificatePdfs;
    private $DmiAdpGrantCertificatePdfs;
    private $DmiECodeGrantCertificatePdfs;
    private $Dmi15DigitGrantCertificatePdfs;
    private $DmiChangeGrantCertificatesPdfs;
    private $DmiSurrenderPdfRecords;
    private $DmiAdpPdfRecords;
    private $DmiECodePdfRecords;
    private $Dmi15DigitPdfRecords;
    private $DmiChangePdfRecords;
    private $DmiApplicationPdfRecords;
    ...
    private $DmiReplicaAllotmentDetails;
    ...
    private $MCommodity;
    private $DmiDocCheckLists;
    private $DmiChemistRegistrations;
    private $DmiAplPdfRecords;
    ...
    private function loadAllModels(): void {
        ...
        $this->DmiOldApplEsignCertLogs = $this->AqcmsWrapper->customeLoadModel('DmiOldApplEsignCertLogs');
        $this->DmiRtiReportPdfRecords = $this->AqcmsWrapper->customeLoadModel('DmiRtiReportPdfRecords');
        $this->DmiSurrenderGrantCertificatePdfs = $this->AqcmsWrapper->customeLoadModel('DmiSurrenderGrantCertificatePdfs');
        $this->DmiAdpGrantCertificatePdfs = $this->AqcmsWrapper->customeLoadModel('DmiAdpGrantCertificatePdfs');
        $this->DmiECodeGrantCertificatePdfs = $this->AqcmsWrapper->customeLoadModel('DmiECodeGrantCertificatePdfs');
        $this->Dmi15DigitGrantCertificatePdfs = $this->AqcmsWrapper->customeLoadModel('Dmi15DigitGrantCertificatePdfs');
        $this->DmiChangeGrantCertificatesPdfs = $this->AqcmsWrapper->customeLoadModel('DmiChangeGrantCertificatesPdfs');
        $this->DmiSurrenderPdfRecords = $this->AqcmsWrapper->customeLoadModel('DmiSurrenderPdfRecords');
        $this->DmiAdpPdfRecords = $this->AqcmsWrapper->customeLoadModel('DmiAdpPdfRecords');
        $this->DmiECodePdfRecords = $this->AqcmsWrapper->customeLoadModel('DmiECodePdfRecords');
        $this->Dmi15DigitPdfRecords = $this->AqcmsWrapper->customeLoadModel('Dmi15DigitPdfRecords');
        $this->DmiChangePdfRecords = $this->AqcmsWrapper->customeLoadModel('DmiChangePdfRecords');
        $this->DmiApplicationPdfRecords = $this->AqcmsWrapper->customeLoadModel('DmiApplicationPdfRecords');
        ...
        $this->DmiApplicantsResetpassKeys = $this->AqcmsWrapper->customeLoadModel('DmiApplicantsResetpassKeys');
        ...
        $this->DmiOnceTokenDetails = $this->AqcmsWrapper->customeLoadModel('DmiOnceTokenDetails');
        ...
        $this->DmiAplFinalSubmits = $this->AqcmsWrapper->customeLoadModel('DmiAplFinalSubmits');
        ...
        $this->DmiReplicaAllotmentDetails = $this->AqcmsWrapper->customeLoadModel('DmiReplicaAllotmentDetails');
        ...
        $this->DmiDocCheckLists = $this->AqcmsWrapper->customeLoadModel('DmiDocCheckLists');
        $this->DmiChemistRegistrations = $this->AqcmsWrapper->customeLoadModel('DmiChemistRegistrations');
        $this->DmiAplPdfRecords = $this->AqcmsWrapper->customeLoadModel('DmiAplPdfRecords');
    }
	...
	public function secondaryHome() {
		...
		//Get the firm type : Akash Thakre [20-09-2023]
		...
		if ($export_unit_status == 'yes' && $firmType) {
			$DmiRenewalFinalSubmits = 'DmiLxrFinalSubmits';
			$DmiRenewalApplicationPdfRecords = 'DmiLxrPdfRecords';
		} else {
			$DmiRenewalFinalSubmits = 'DmiRenewalFinalSubmits';
			$DmiRenewalApplicationPdfRecords = 'DmiRenewalApplicationPdfRecords';
		}
        ...
        $DmiRenewalApplicationPdfRecords = $this->AqcmsWrapper->customeLoadModel($DmiRenewalApplicationPdfRecords);
		...
		//// For Applications PDFs on Dashboard /////
			...
			#Renewal Application (2) /Application PDF/ // Changed the query for lab export flow (LXR) : Akash Thakre [20-09-2023]
		...
		//// Grant PDFs for Applications ////
			...
			#Renewal Application (2) /GRANT PDF/

			$renewal_grant_pdfs = $this->DmiGrantCertificatesPdfs->find('all', array('conditions' => array('customer_id IS' => $customer_id)))->toArray();
			$this->set('renewal_grant_pdfs', $renewal_grant_pdfs);
		...
		//// Final Sumbit Statuses Entries for All Applications ////
			...
			//Update : The model name is now taken dynamically according to the lab export renewal application : Akash Thakre [20-09-2023]
		...
		//// For Rejected / Suspension / Surrender / Showcause Notices  by Akash////
            ...
            $this->_performAppealRelatedActivityInSecondaryHome($customer_id);
		...
		//check if appl. renewed with MSEO from BEVO
		//on 21-09-2023 by Amol, to show note on applicant dashboard.

		$checkMseologs = $this->DmiRenBevoToMseoLogs->find('all',array('conditions'=>array('customer_id IS'=>$customer_id)))->first();
		$this->set('checkMseologs', $checkMseologs);
	...
	public function documentCheckList()
	{
		...
		$form_type = ['CA Non Bevo','Printing Press','Laboratory Domestic','15 Digit Code','E code', 'Laboratory Export','CA Export','CA MSEO'];
		$this->set('form_type', $form_type);
		$comm_array = [];
		$i = 0;
		foreach($form_type as $eachformtype){
			$get_doc =  $this->DmiDocCheckLists->find('all')->select(['releted_document','pdf_file'])->where(['form_type'=>$eachformtype])->order('id ASC')->first();
			$doc_array[$i] = $get_doc;
			$i++;
		}
		...
		if(null!== ($this->request->getData('save'))){
			...
			if(!empty($grantData)){
				...
				foreach ($grantData as $key => $grantdata) {
					...
					//Joshi, Akash - Commenting below code as below referred API is not present in live.
					//$cancellation_record = $this->Customfunctions->isApplicationCancelled($grantdata['customer_id']);
					//Joshi, AKash please comment below if uncomment above line.
					 $cancellation_record='';
			...
			if($maptype == "lab"){
				...
				if(!empty($get_record_pp)){
					...
					$DmiCaPpLabMapings = $this->DmiCaPpLabMapings->newEntity(array(
						...
						'customer_id'=>$customer_id,
						'pp_id'=>$pp_id,
						// 'lab_id'=>$lab_id, // commented by shankhpal on 18/09/2023
						'map_type'=> $maptype,
						'created'=>date('Y-m-d H:i:s'),
						'modified'=>date('Y-m-d H:i:s'),
					));
    ...
    private function _performAppealRelatedActivityInSecondaryHome($customer_id){
			...
			if (!empty($final_appeal_submit_data)) {
			   ...
			   // Find the latest final submitted appeal
				...
				if (!empty($latestFinalSubmittedAppeal)) {
    				...
    				$InProcessAppeal = $this->DmiAplFormDetails->find('all', [
    			    'conditions' => [
