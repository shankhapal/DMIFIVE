...
class ReportsController extends AppController {
    ...
    private $UserRole;
    private $DmiUserRolesManagmentLogs;
    private $DmiMoAllocationLogs;
    private $DmiGrantCertificatesPdfs;
    private $DmiIoAllocationLogs;
    private $DmiRoAllocationLogs;
    private $DmiStates;
    ...
    private function loadAllModels(): void {
        ...
        $this->UserRole = $this->AqcmsWrapper->customeLoadModel('UserRole');
        $this->DmiUserRolesManagmentLogs = $this->AqcmsWrapper->customeLoadModel('DmiUserRolesManagmentLogs');
        $this->DmiUserRoles = $this->AqcmsWrapper->customeLoadModel('DmiUserRoles');
        $this->DmiMoAllocationLogs = $this->AqcmsWrapper->customeLoadModel('DmiMoAllocationLogs');
        $this->DmiGrantCertificatesPdfs = $this->AqcmsWrapper->customeLoadModel('DmiGrantCertificatesPdfs');
        $this->DmiIoAllocationLogs = $this->AqcmsWrapper->customeLoadModel('DmiIoAllocationLogs');
        $this->DmiRoAllocationLogs = $this->AqcmsWrapper->customeLoadModel('DmiRoAllocationLogs');
        ...
        $this->DmiSentEmailLogs = $this->AqcmsWrapper->customeLoadModel('DmiSentEmailLogs');
		...
		if (null !== ($this->request->getData('search'))) {
			...
			if (!empty($from_date) && !empty($to_date)) {
				...
				foreach ($applications_current_positions_tables as $each_table) {
					...
					$this->AqcmsWrapper->customeLoadModel($each_table);
					$this->AqcmsWrapper->customeLoadModel($key);
				...
				//sum for adv Advance application by Vikas on date [25-09-2023]
				...
				//sum for adv change application by Vikas on date [25-09-2023]
				...
				//sum for adv Chemist application by Vikas on date [25-09-2023]
				...
				//sum for adv renewal application by Vikas on date [18-09-2023]
				$changeApplicationrevenue_Query =$this->DmiChangePaymentDetails->find('all')->where($searchPendingConditions)
						->where(['payment_confirmation' => 'confirmed'])->sumOf('amount_paid');
				$changeApplicationrevenue = ['sum' => $changeApplicationrevenue_Query];

				//sum of chemist application by Vikas on date [18-09-2023]
				$chemistApplicationrevenue_Query =$this->DmiChemistPaymentDetails->find('all')->where($searchPendingConditions)
						->where(['payment_confirmation' => 'confirmed'])->sumOf('amount_paid');
				$chemistApplicationrevenue = ['sum' => $chemistApplicationrevenue_Query];
			...
			// added by vikas on date [18-09-2023]
			...
			//added sum of chemist, change and adv by Vikas on date [25-09-2023]
			$totalrevenue = $FrontStatisticsController->thousandsCurrencyFormat($newApplicationrevenue['sum'] +$renewalApplicationrevenue['sum'] +$changeApplicationrevenue['sum'] +$chemistApplicationrevenue['sum'] +$advApplicationrevenue['sum']);
			...
			//To show the total amount statiscits after filter added by Vikas on date [28-09-2023]
	...
	// Date : ----
		...
		$ro_office = $this->DmiRoOffices->getAllOffices();
	...
	public function approvedApplicationSearchConditions ($search_application_type_id, $application_approved_office, $search_from_date, $search_to_date, $table, $search_flag,$data_id=null,$approved_application_type=null)
	{
		...
		///==========Modified by Vikas on 28-Nov-2023 START Search Select Office Filter filter =============
		...
		{
			// Collect the short code of office by id by Vikas on 28-11-2023
			$search_ro_office_id = array();
			foreach ($application_approved_office as $value) {
				$ro_office_id = $value;
				$get_short_code = $this->DmiRoOffices->find('all', array( 'fields' => 'short_code',  'conditions' => array('id' => $ro_office_id) ))->first();

				 if (!empty($get_short_code)) {
				        $search_ro_office_id[] = $get_short_code->short_code;
				    }
			}
			...
			foreach ($approved_application_customer_id as $each_customer_id)
			{
				if (!empty($each_customer_id['customer_id']))
				{
					$explode = explode("/",$each_customer_id['customer_id']);

					if(in_array($explode[2],$search_ro_office_id))
					{
						$approved_application_list[$i] = $each_customer_id['customer_id'];
		...
		///========== Modified by Vikas on 28-Nov-2023 End Search Select Office Filter filter =======
		...
		///====== Vikas  28-Nov-2023 Start Search by Application Type and  Office Filter filter ==========
		elseif ($search_application_type_id != '' && $application_approved_office != '' && $search_from_date =='' && $search_to_date == '' )
		{
			// Collect the short code of office by id by Vikas on 28-11-2023
			$search_ro_office_id = array();
		...
		///====== Vikas 28-Nov-2023 END Search by Application Type and  Office Filter filter ==========

		// Start Search filter Application Type  and Date range filter
		elseif ($search_application_type_id != '' && $application_approved_office == '' && $search_from_date !='' && $search_to_date != '') {
		...
		// End Search filter Application Type  and Date range filter

		//Start Filter all optoins of office Type, RO Office and Date Range
		//Modified By vikas on 28-11-2023
		elseif ($search_application_type_id != '' && $application_approved_office != '' && $search_from_date !='' && $search_to_date != '') {
		...
		// End Filter all optoins of office Type, RO Office and Date Range
	...
	public function paymentDetailsReport($applicn_type) {
		...
		//new model according to application type by Vikas on date [18-09-2023]
		...
		$search_from_date='';
		$search_to_date='';
		$renewal_firm_customer_id_list ='';
		$change_payment_customer_id_list ='';
		$chemist_payment_customer_id_list ='';
		$adv_payment_customer_id_list ='';
		...
		$conn = ConnectionManager::get('default'); // Added by Vikas [14/09/20223]
		...
		if (null!==($this->request->getData('search_logs')))
		{
			...
			if(!empty($report_for_array_select))
			{
			...
			$chemistdate_not_empty  = array(); // Add By Vikas on Date [20-09-2023]
			...
			$ro_office_not_empty_counts=''; // This variable for Custome Count Created By Vikas 13-09-2023
			$datarange_not_empty =''; // This variable for Custome Count Created By Vikas 13-09-2023
			$where_condition  =''; // This variable for Custome Count Created By Vikas 13-09-2023
			$count_conditions  = []; // This variable for Custome Count Created By Vikas 13-09-2023
			$whereClause=[];  // This variable for Custome Count Created By Vikas 13-09-2023


			if ($application_type != '') {

				$application_type_not_empty = ['certification_type IS' => $application_type];
				$count_conditions[] = "certification_type = '$application_type'";  // This variable for Custome Count Created By Vikas 13-09-2023
			}

			if ($ro_office != '') {
				$ro_office_not_empty = ['DmiDistricts.ro_id IS' => $ro_office];
				$ro_office_not_empty_counts = " AND DmiDistricts.ro_id='$ro_office'"; // This variable for Custome Count Created By Vikas 13-09-2023
			}

			if ($state != '') {
				$state_not_empty = ['state IS' => $state];
				$count_conditions[] = "state = '$state'"; // This variable for Custome Count Created By Vikas 13-09-2023
			}

			if ($district != '') {
				$district_not_empty = ['district IS' => $district];
				$count_conditions[] = "district = '$district'"; // This variable for Custome Count Created By Vikas 13-09-2023
			}

			// Date Range custome fileter Define here By vikas 13-09-2023
			if ($search_from_date != '' && $search_to_date != '') {
				...
				$datarange_not_empty = " AND date(transaction_date) BETWEEN '$search_from_date' AND '$search_to_date'";
				$chemistdate_not_empty = "date(transaction_date) BETWEEN '$search_from_date' AND '$search_to_date'";
			}

			if($report_for == 4)
			{
				$count_conditions[] = "DmiChemistPaymentDetails.payment_confirmation = 'confirmed'";
				if (!empty($chemistdate_not_empty)) {
					$count_conditions[] = $chemistdate_not_empty;
				}
			}

			//  concatinate the custom  where cluse which is used for Total grands counts.
			$whereClause = implode(' AND ', $count_conditions);

			if(!empty($whereClause))
			{
				$where_condition = "WHERE ($whereClause)";
			}

			// Grand Total counts as per the Search filter By vikas 18-09-2023
			if($report_for == 1){

				$newApplicationrevenue_Query = 0;
				$newApplicationAmount = $conn->execute("SELECT SUM(CAST(DmiApplicantPaymentDetails.amount_paid AS DECIMAL(10, 2))) AS total_sum
				FROM dmi_firms DmiFirms INNER JOIN dmi_districts DmiDistricts
				ON (DmiDistricts.id = DmiFirms.district::integer $ro_office_not_empty_counts)
				INNER JOIN dmi_applicant_payment_details DmiApplicantPaymentDetails
				ON (DmiApplicantPaymentDetails.customer_id = DmiFirms.customer_id $datarange_not_empty AND payment_confirmation = 'confirmed')
				$where_condition

				")->fetch('assoc');

				if (!empty($newApplicationAmount)) {
					$newApplicationrevenue_Query= $newApplicationAmount['total_sum'];
				}
			...
			// Grand Total counts as per the Search filter For Renewal Application By vikas [18-09-2023]
			elseif($report_for == 2){

				$renewalApplicationrevenue_Query = 0;
				$renewalApplicationAmount = $conn->execute("
				SELECT SUM(CAST(DmiRenewalApplicantPaymentDetails.amount_paid AS DECIMAL(10, 2))) AS total_sum
				FROM dmi_firms DmiFirms INNER JOIN dmi_districts DmiDistricts
				ON (DmiDistricts.id = DmiFirms.district::integer $ro_office_not_empty_counts)
				INNER JOIN dmi_renewal_applicant_payment_details DmiRenewalApplicantPaymentDetails
				ON (DmiRenewalApplicantPaymentDetails.customer_id = DmiFirms.customer_id $datarange_not_empty AND  payment_confirmation = 'confirmed')
				$where_condition")->fetch('assoc');

				if (!empty($renewalApplicationAmount)) {
					$renewalApplicationrevenue_Query= $renewalApplicationAmount['total_sum'];
				}
			...
			//sum of revenue change application Revenue by Vikas on date [20-09-2023]
			elseif($report_for == 3){

				$ChangeApplicationrevenue_Query = 0;
				$ChangeApplicationAmount = $conn->execute("
				SELECT SUM(CAST(DmiChangePaymentDetails.amount_paid AS DECIMAL(10, 2))) AS total_sum
				FROM dmi_firms DmiFirms INNER JOIN dmi_districts DmiDistricts
				ON (DmiDistricts.id = DmiFirms.district::integer $ro_office_not_empty_counts)
				INNER JOIN dmi_change_payment_details DmiChangePaymentDetails
				ON (DmiChangePaymentDetails.customer_id = DmiFirms.customer_id
				$datarange_not_empty AND payment_confirmation = 'confirmed')
				$where_condition")->fetch('assoc');

				if (!empty($ChangeApplicationAmount)) {
					$changeApplicationrevenue_Query = $ChangeApplicationAmount['total_sum'];
				}
			...
			//Grand Total of chemist application by vikas on date [20-09-2023]
			elseif(!empty($report_for) && $report_for == 4){
				$chemistPaymentDetailsTable = TableRegistry::getTableLocator()->get('DmiFirms');
				$chemistApplicationrevenue_Query = 0;
				$chemistApplicationAmount = $conn->execute("
				SELECT SUM(CAST(DmiChemistPaymentDetails.amount_paid AS DECIMAL(15, 2))) AS total_sum
				FROM dmi_chemist_payment_details DmiChemistPaymentDetails
				INNER JOIN dmi_chemist_registrations DmiChemistRegistrations
				ON DmiChemistRegistrations.chemist_id = DmiChemistPaymentDetails.customer_id
				INNER JOIN dmi_firms DmiFirms ON DmiFirms.customer_id = DmiChemistRegistrations.created_by
				INNER JOIN dmi_districts DmiDistricts
				ON (DmiDistricts.id = DmiFirms.district::integer $ro_office_not_empty_counts)
				$where_condition")->fetch('assoc');

				if (!empty($chemistApplicationAmount)) {
					$chemistApplicationrevenue_Query= $chemistApplicationAmount['total_sum'];
				}
			...
			// Grand Total counts as per the Search filter For Advance Payment By vikas [18-09-2023]
			elseif($report_for == 7){
				$advApplicationrevenue_Query = 0;
				$advApplicationAmount = $conn->execute("
				SELECT SUM(CAST(DmiAdvPaymentDetails.amount_paid AS DECIMAL(10, 2))) AS total_sum
				FROM dmi_firms DmiFirms INNER JOIN dmi_districts DmiDistricts
				ON (DmiDistricts.id = DmiFirms.district::integer $ro_office_not_empty_counts)
				INNER JOIN dmi_adv_payment_details DmiAdvPaymentDetails
				ON (DmiAdvPaymentDetails.customer_id = DmiFirms.customer_id $datarange_not_empty AND  payment_confirmation = 'confirmed')
				$where_condition")->fetch('assoc');

				if (!empty($advApplicationAmount)) {
					$advApplicationrevenue_Query= $advApplicationAmount['total_sum'];
				}
			...
			// TOTAL COUNT serach wise move for apply the conditon filter wise total by Vikas 12-09-2023
			...
			if ($application_type != '' || $ro_office != '' || $state != '' || $district != '' || $search_from_date != '' || $search_to_date != '')
			{
				...
				if ($search_from_date != '' && $search_to_date != '')
				{
					...
					if($report_for == 3)
					{
						$change_payment_customer_id_list = 	$this->DmiFirms->find('all')
															->select(['id', 'customer_id'])
															->join(['DmiDistricts' => ['table' => 'dmi_districts', 'type' => 'INNER',
																	'conditions' => ['DmiDistricts.id = DmiFirms.district::integer', $ro_office_not_empty]],
																	'DmiChangePaymentDetails' => ['table' => 'dmi_change_payment_details', 'type' => 'INNER',
																	'conditions' => ['DmiChangePaymentDetails.customer_id = DmiFirms.customer_id', $date_not_empty,'payment_confirmation' => 'confirmed']]])
															->select(['id','customer_id'])
															->where(array_merge($application_type_not_empty, $state_not_empty, $district_not_empty))
															->bind(':start', $search_from_date, 'date')->bind(':end', $search_to_date, 'date')
															->combine('id', 'customer_id')
					...
					if($report_for == 4)
					{
						$chemist_payment_customer_id_list =  $this->DmiChemistPaymentDetails->find('all')
							->select(['id', 'customer_id'])
							->join([
									'DmiChemistRegistrations' => [
									'table' => 'dmi_chemist_registrations', 'type' => 'INNER',
									'conditions' => ['DmiChemistRegistrations.chemist_id = DmiChemistPaymentDetails.customer_id']],
									'DmiFirms' => [
										'table' => 'dmi_firms', 'type' => 'INNER',
										'conditions' => ['DmiFirms.customer_id = DmiChemistRegistrations.created_by']
									],
									'DmiDistricts' => [
									'table' => 'dmi_districts', 'type' => 'INNER',
									'conditions' => ['DmiDistricts.id = DmiFirms.district::integer', $ro_office_not_empty]]])
							->select(['id','customer_id'])
							->where(['DmiChemistPaymentDetails.payment_confirmation' => 'confirmed'])
							->where(array_merge($application_type_not_empty, $state_not_empty, $district_not_empty))
							->where($date_not_empty)
							->bind(':start', $search_from_date, 'date')->bind(':end', $search_to_date, 'date')
							->combine('id', 'customer_id')
							->toArray();
					}
					if($report_for == 7)
					{
						$adv_payment_customer_id_list = $this->DmiFirms->find('all')
																	->select(['id', 'customer_id'])
																	->join(['DmiDistricts' => ['table' => 'dmi_districts', 'type' => 'INNER',
																			'conditions' => ['DmiDistricts.id = DmiFirms.district::integer', $ro_office_not_empty]],
																			'DmiAdvPaymentDetails' => ['table' => 'dmi_adv_payment_details', 'type' => 'INNER',
																			'conditions' => ['DmiAdvPaymentDetails.customer_id = DmiFirms.customer_id',$renewal_date_not_empty, 'payment_confirmation' => 'confirmed']]])
																	->select(['id','customer_id'])
																	->where(array_merge($application_type_not_empty, $state_not_empty, $district_not_empty))
																	->bind(':start', $search_from_date, 'date')->bind(':end', $search_to_date, 'date')
																	->combine('id', 'customer_id')
				...
				// Without Date Wise Filter Records.
				else
				{
					if($report_for == 1)
					{
					...
					if($report_for == 3)
					{
						$change_payment_customer_id_list = $this->DmiFirms->find('all')
														...
														->join(['DmiDistricts' => ['table' => 'dmi_districts', 'type' => 'INNER',
																'conditions' => ['DmiDistricts.id = DmiFirms.district::integer', $ro_office_not_empty]],
																'DmiChangePaymentDetails' => ['table' => 'dmi_change_payment_details', 'type' => 'INNER',
																'conditions' => ['DmiChangePaymentDetails.customer_id = DmiFirms.customer_id','payment_confirmation' => 'confirmed']]])
														->select(['id','customer_id'])
														->where(array_merge($application_type_not_empty, $state_not_empty, $district_not_empty))
														->combine('id', 'customer_id')
														->toArray();
					...
					if($report_for == 7)
					{
						$adv_payment_customer_id_list = $this->DmiFirms->find('all')
														...
														->join(['DmiDistricts' => ['table' => 'dmi_districts', 'type' => 'INNER',
																'conditions' => ['DmiDistricts.id = DmiFirms.district::integer', $ro_office_not_empty]],
																'DmiAdvPaymentDetails' => ['table' => 'dmi_adv_payment_details', 'type' => 'INNER',
																'conditions' => ['DmiAdvPaymentDetails.customer_id = DmiFirms.customer_id','payment_confirmation' => 'confirmed']]])
														->select(['id','customer_id'])
														->where(array_merge($application_type_not_empty, $state_not_empty, $district_not_empty))
														->combine('id', 'customer_id')
														->toArray();
				...
				// Renewal Payment Details Customer Id Select here By Vikas [18-09-2023]
				if ($renewal_firm_customer_id_list != null) {
					$renewal_firm_customer_id_condition = ['customer_id IN' => $renewal_firm_customer_id_list];
				} else {
					$renewal_firm_customer_id_condition = ['customer_id IS' => ''];
				}
				// Change Payment Details Customer Id Select here By Vikas [18-09-2023]
				if ($change_payment_customer_id_list != null) {
					$change_payment_customer_id_condition = ['customer_id IN' => $change_payment_customer_id_list];
				} else {
					$change_payment_customer_id_condition = ['customer_id IS' => ''];
				}

				// Chemist Payment Details Customer Id Select here By Vikas [18-09-2023]
				if ($chemist_payment_customer_id_list != null) {
					$chemist_payment_customer_id_condition = ['customer_id IN' => $chemist_payment_customer_id_list];
				} else {
					$chemist_payment_customer_id_condition = ['customer_id IS' => ''];
				}

				// Advance Payment Details Customer Id Select here By Vikas [18-09-2023]
				if ($adv_payment_customer_id_list != null) {
					$adv_payment_customer_id_condition = ['customer_id IN' => $adv_payment_customer_id_list];
				} else {
					$adv_payment_customer_id_condition = ['customer_id IS' => ''];
				}


			} else {
				$firm_customer_id_condition = array();
				$renewal_firm_customer_id_condition = array();
				$change_payment_customer_id_condition = array(); // Add for Change Payement customer Id by Vikas [18-09-2023]
				$chemist_payment_customer_id_condition = array(); // Add for Change Payement customer Id by Vikas [18-09-2023]
				$adv_payment_customer_id_condition = array(); // Add for Advance Payement customer Id by Vikas [18-09-2023]
			}
		}
		else
		{
			$firm_customer_id_condition = array();
			$renewal_firm_customer_id_condition = array();
			$change_payment_customer_id_condition = array();
			$chemist_payment_customer_id_condition = array();
			$adv_payment_customer_id_condition = array(); // add for Advance payment detial By Vikas [18-09-2023]

			//Grand Total of New Application revenue by Vikas on date [18-09-2023]
			...
			//Grand Total of Renewal Application revenue by Vikas on date [18-09-2023]
			...
			//Grand Total of revenue change application by Vikas on date [18-09-2023]
			...
			//Grand Total of chemist application by vikas on date [18-09-2023]
			elseif(!empty($report_for) && $report_for == 4){
			...
			//sum for adv application Revenue by shreeya on date [23-06-2023]
			...
			// Get Application ID and Application Type  list here. comment by Vikas [14-09-2023]
			$application_array = $this->DmiApplicationTypes->find('all')->select(['id', 'application_type'])->where(['delete_status IS NULL'])->order(['id'])->combine('id', 'application_type')->toArray();
			...
			if($reportForNull == 'yes')
			{
				...
				foreach ($flowwise_table_data as $FlowWise_Tables){
					...
					$this->AqcmsWrapper->customeLoadModel($payment_table);
					...
					if($report_for == 2)
					{
						$customer_id_list  = $query_cil->select(['customer_id', 'max' => $query_cil->func()->max('certificate_type')])
										->distinct()
										->where($renewal_firm_customer_id_condition)
										->where(['payment_confirmation' => 'confirmed'])
										->group(['customer_id'])->order(['MAX(certificate_type)'])
										->toArray();
					}
					if($report_for == 1)
					{
					...
					{
						$customer_id_list = $query_cil->select(['customer_id', 'max' => $query_cil->func()->max('certificate_type')])
										->distinct()->where($change_payment_customer_id_condition)
										->group(['customer_id'])->order(['MAX(certificate_type)'])->toArray();
					}
					if($report_for == 4)
					{
						$customer_id_list = $query_cil->select(['customer_id', 'max' => $query_cil->func()->max('certificate_type')])
										->distinct()
										->where($chemist_payment_customer_id_condition)
										->group(['customer_id'])->order(['MAX(certificate_type)'])->toArray();
					}
					if($report_for == 7)
					{
						...
						$query_cil = $this->$payment_table->find('all');
						$customer_id_list = $query_cil->select(['customer_id', 'max' => $query_cil->func()->max('certificate_type')])
										->distinct()->where($adv_payment_customer_id_condition)
										->group(['customer_id'])->order(['MAX(certificate_type)'])->toArray();
					}

					// To print the New Applicant Revenue detials Show Customer
					$k=0;
					foreach ($customer_id_list as $customer_id) {
						...
						$customer_payment_id_list = $this->$payment_table->find('all')->select(['id'])->where(['customer_id' => $customer_id['customer_id'], 'payment_confirmation' => 'confirmed'])->toArray();
						 ...
						 if (!empty($customer_payment_id_list)) {
							...
							foreach ($customer_payment_id_list as $key => $value) {
								$payment_max_id[$i][$k] = $value->id;
								$chemistPaymentDetailsTable = TableRegistry::getTableLocator()->get('DmiFirms');
							// Find all the payment details by compare by ID of payment table vikas 14-09-2023
							...
							// Find the Details of Firms Detils save by compare of customer id comment by Vikas [14-09-2023]
							...
							// Application Type Stored application Type here.. 14-09-2023 commnet By Vikas
							$apl_type_res[$i][$k] = $apl_type_res1['application_type'];

							// Find district id only here Stored  here.. 14-09-2023 commnet By Vikas
					...
					// again Collect here  flow wise all payment table with application type id
					...
					foreach ($payment_data as $payment_value) {
						...
						$this->AqcmsWrapper->customeLoadModel($tbl_data);
		...
		$this->set('report_for',$report_for); // variable set for Application type
