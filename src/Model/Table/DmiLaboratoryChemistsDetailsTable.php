...
class DmiLaboratoryChemistsDetailsTable extends Table{
	...
	public function laboratoryChemistDetails(){
		...
		//changed the query for the lab export application 
		//if the lab export application is applied show only the fruits and vegetable category's commodity
		// : Akash Thakre [12-09-2023] (LXR)
		if ($export_unit_status == 'yes') {
			$show_chemist_commodity_types = $M_commodity
					->find('all')->select(['commodity_code', 'commodity_name'])
					->where(['category_code' => 14])->combine('commodity_code', 'commodity_name')->toArray();
