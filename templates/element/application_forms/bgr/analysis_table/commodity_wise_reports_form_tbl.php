         ...
         <caption></caption>
              ...
              <tr>
                  ...
                  <th
                      ...
                      scope="col">
                      Name :
                      <?php echo isset($section_form_details[1]) ? $section_form_details[1] : "NA"; ?>,
                      Email :
                      <?php echo base64_decode(isset($section_form_details[2]) ? $section_form_details[2] : "NA"); ?>,
                      Address :
                      <?php echo isset($section_form_details[3]) ? $section_form_details[3] : "NA"; ?>
                  </th>
              </tr>
              <tr>
                  ...
                  <?php
                    // if(empty($section_form_details[12]) || !empty($section_form_details[16])){
                          ...
                          if(!empty($section_form_details[12])){
                            ...
                            $unitCode = $eachrow['packetsizeunit'];
                            // $get_unit = "SELECT unit from dmi_replica_unit_details WHERE commodity_code = $eachrow['packetsizeunit']";

                            $get_unit_query = "SELECT sub_unit FROM dmi_replica_unit_details WHERE id = $unitCode";

                            $unitdata = $conn->execute($get_unit_query);
                            $row5 = $unitdata->fetch();

                             if(!empty($row5)){
                               $unitname = $row5[0]; // Use index 0 to access the value
                            }
                              ...
                              <tr  id="custom_row<?php echo $eachrow['id'];?>">
                                ...
                                <!-- //display date in dd/mm/yyyy format added by shankhpal on 12/12/23 -->
                                <td><?php echo date('d/m/Y', strtotime($eachrow['datesampling'])); ?></td>
                                <td><?php echo date('d/m/Y', strtotime($eachrow['dateofpacking'])); ?></td>
                          ...
                          <tr id="add_new_row">
                              ...
                              <td>
                                <?php
                                  ...
                                  $gradingLab = $eachrow1['grading_lab'];
                                  if (strpos($gradingLab, "/Own") !== false) {
                                      $labDetails = $DmiCaMappingOwnLabDetails->find()
                                          ->select(['lab_name'])
                                          ->where(['own_lab_id' => $gradingLab])
                                          ->first();
                                      $laboratoryName = $labDetails ? $labDetails->lab_name : null;
                                  } else {
                                      $fetchLaboratoryDetailData = $DmiCustomerLaboratoryDetails->find()
                                          ->select(['laboratory_name'])
                                          ->where(['id' => $gradingLab])
                                          ->first();
                                      $laboratoryName = $fetchLaboratoryDetailData ? $fetchLaboratoryDetailData->laboratory_name : null;
                                  }
...
<input type="hidden" id="selected_period" value="<?php echo $section_form_details[18]; ?>">
