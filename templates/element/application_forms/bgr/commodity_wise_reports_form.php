...
<?php echo $this->Form->create(null,array('type'=>'file', 'enctype'=>'multipart/form-data', 'id'=>$section)); ?>
 <input type="hidden" id="status" value='<?php echo isset($checkIfgrant['status'])?$checkIfgrant['status']:""; ?>'>
		...
		<?php } ?>
			<div id="form_inner_main" class="card card-success">
				<?php if(!empty($checkIfgrant) && $checkIfgrant['status'] == 'Granted'){ ?>
					...
					<?php }else {
						if($_SESSION['current_level'] == 'applicant'){
						?>

						<button type="button" class="btn btn-primary btn-sm help" data-toggle="modal" data-target="#exampleModal">
					...
					<?php } }?>
						 ...
						 <div class="tank_table form-horizontal " >
							<div class="card-body">
								<?php
									if(!isset($reportStatus)){ ?>
								        ...
								        <?php }elseif(isset($reportStatus) && $reportStatus['form_status'] == 'approved'){ ?>
										<table class="table">
												<thead>
													<tr>
														<th scope="col">#</th>
												...
												<tbody>
													...
													<tr>
														<?php if(!empty($reportStatus)){ ?>
																<th scope="row">1</th>
																<td><?php echo $reportStatus['customer_id']; ?></td>
																<td>
																	<?php
																		$period_from = $reportStatus['period_from'];
																		$period_to = $reportStatus['period_to'];
							...
							<?php
								if($_SESSION['current_level'] == 'applicant'){
								echo $this->Html->link('Back', ['controller' => 'chemist', 'action' => 'display-mapped-c-a-dropdown'], ['class' => 'btn btn-secondary float-right mr-2']);
								}
								?>
            </div>
					</div>
  ...
  <?php
		...
		echo $this->Html->script('element/application_forms/bgr/bgr_replica_serial_appl_table');
